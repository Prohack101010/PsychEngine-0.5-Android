...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		var buttonWS = #if mobile 'UP/Down' #else 'W/S or Mouse Wheel' #end;
		var buttonAD = #if mobile 'Left/Right' #else 'A or Left/D or Right' #end;
		var buttonSHIFT = #if mobile 'C' #else 'Shift' #end;
		var buttonZX = #if mobile 'V/D' #else 'Z/X' #end;
		var buttonESC = #if mobile 'B' #else 'Esc' #end;
		var buttonENTER = #if mobile 'A' #else 'Enter' #end;
		var buttonQE = #if mobile 'UP/Down(Right Side)' #else 'Q/E' #end;
		var buttonSPACE = #if mobile 'Y' #else 'Space' #end;
		var buttonR = #if mobile 'Z' #else 'R' #end;
		var Conductor = "Conductor's";
		var buttonCONTROL = #if mobile 'C' #else 'Control' #end;

		text =
		'$buttonWS - Change $Conductor strum time
		\n$buttonAD - Go to the previous/next section
		\nHold $buttonSHIFT to move 4x faster
		\nHold $buttonCONTROL and click on an arrow to select it
		\n$buttonZX - Zoom in/out
		\n
		\n$buttonESC - Test your chart inside Chart Editor
		\n$buttonENTER - Play your chart
		\n$buttonQE - Decrease/Increase Note Sustain Length
		\n$buttonSPACE - Stop/Resume song';
	...
	override function update(elapsed:Float)
	{
		...
		for (touch in FlxG.touches.list)
		{
			...
			if (touch.justPressed)
			{
				...
				else if (!_virtualpad.buttonC.pressed)
				{
					if (touch.x > gridBG.x
						&& touch.x < gridBG.x + gridBG.width
						&& touch.y > gridBG.y
		...
		if (!blockInput)
		{
			...
			if (#if mobile _virtualpad.buttonA.justPressed || #end FlxG.keys.justPressed.ENTER)
			{
				autosaveSong();
			...
			if(curSelectedNote != null && curSelectedNote[1] > -1) {
				if (#if mobile _virtualpad.buttonE.justPressed || #end FlxG.keys.justPressed.E)
				{
					changeNoteSustain(Conductor.stepCrochet);
				}
				if (#if mobile _virtualpad.buttonQ.justPressed || #end FlxG.keys.justPressed.Q)
				{
					changeNoteSustain(-Conductor.stepCrochet);
				}
			}

			if((#if mobile _virtualpad.buttonV.justPressed || #end FlxG.keys.justPressed.Z) && curZoom > 0) {
				--curZoom;
				updateZoom();
			}
			if((#if mobile _virtualpad.buttonD.justPressed || #end FlxG.keys.justPressed.X) && curZoom < zoomList.length-1) {
				curZoom++;
				updateZoom();
			}
			...
			if (#if mobile _virtualpad.buttonY.justPressed || #end FlxG.keys.justPressed.SPACE)
			{
			...
			if (#if mobile _virtualpad.buttonZ.justPressed || #end FlxG.keys.justPressed.R)
			{
				if (FlxG.keys.pressed.SHIFT)
					resetSection(true);
				else
			...
			if ((FlxG.keys.pressed.W || FlxG.keys.pressed.S) #if mobile || (_virtualpad.buttonUp.pressed || _virtualpad.buttonDown.pressed) #end)
			{
				...
				if (#if mobile _virtualpad.buttonUp.pressed || #end FlxG.keys.pressed.W)
				{
			...
			if (#if mobile _virtualpad.buttonC.pressed || #end FlxG.keys.pressed.SHIFT)
				shiftThing = 4;
			#if mobile
	...
	function loadAudioBuffer() {
		...
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(Paths.modsSongs(currentSongName + '/Inst'))) {
			audioBuffers[0] = AudioBuffer.fromFile(Paths.modsSongs(currentSongName + '/Inst'));
		...
		audioBuffers[1] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(Paths.modsSongs(currentSongName + '/Voices'))) {
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if ((#if mobile _virtualpad.buttonC.pressed || #end FlxG.keys.pressed.CONTROL) && noteData > -1)
		{
	...
	private function saveEvents()
	{
		...
		{
			#if mobile
			StorageUtil.saveContent(Paths.formatToSongPath(_song.song) + "-events.json", data.trim());
			#else
			_file = new FileReference();
			_file.addEventListener(Event.COMPLETE, onSaveComplete);
			_file.addEventListener(Event.CANCEL, onSaveCancel);
